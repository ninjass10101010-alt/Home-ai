  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (meal) {
        ...
        // Update meal plan entry if it exists
        try {
          mealPlanEntry = await pb.collection("meal_plan_entries")
            .getFirstListItem(`recipeId = "${savedMeal.recipeId || savedMeal.id}"`);
          if (mealPlanEntry) {
          ...
          // Meal plan entry doesn't exist, that's ok
        }
      } else {
        savedMeal = await pb.collection("meals").create(formData);

        // Create meal plan entry for new meals
        try {
          mealPlanEntry = await pb.collection("meal_plan_entries").create({
            recipeId: savedMeal.recipeId || savedMeal.id, // Fallback for legacy meals
            scheduledFor: formData.date,
            servings: formData.servings || 4,
            notes: formData.description,
            autoGenerated: false,
            lastSyncedAt: new Date().toISOString(),
          ...
          console.error("Failed to create meal plan entry:", e);
        }
      }

      // Use the new sync service if we have a meal plan entry
      if (syncToGrocery && mealPlanEntry) {
        await MealPlanningSync.syncMealPlanToGrocery(mealPlanEntry);
      } else if (syncToGrocery && formData.ingredients && savedMeal) {
        // Fallback to legacy sync for meals without structured recipes
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-text-muted uppercase tracking-wider">Servings</label>
          </div>
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => setFormData({ ...formData, servings: Math.max(1, (formData.servings || 4) - 1) })}
              className="w-10 h-10 rounded-xl bg-surface-2 border border-surface-3 flex items-center justify-center text-text-muted hover:text-text-primary transition-colors"
            >
            ...
            <button
              type="button"
              onClick={() => setFormData({ ...formData, servings: Math.min(12, (formData.servings || 4) + 1) })}
              className="w-10 h-10 rounded-xl bg-surface-2 border border-surface-3 flex items-center justify-center text-text-muted hover:text-text-primary transition-colors"
            >
              <span className="text-lg font-bold">+</span>
            </button>
