...
export function wearAdvice(feelsLikeF: number, precipProb: number | null, isKid: boolean, knownWet = false): WearAdvice {
  const cold = feelsLikeF < 40;
  const chilly = feelsLikeF < 55;
  const hot = feelsLikeF >= 90;
  const wet = knownWet || (precipProb != null && precipProb >= 40);
