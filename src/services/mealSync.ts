...
export class MealSyncService {
  ...
  private async getScheduledMeals(userId: string): Promise<Meal[]> {
    try {
      const fromDb = await db.selectMeals();
  ...
  private async getPantryItems(userId: string): Promise<PantryItem[]> {
    try {
      const items = await db.selectPantry();
  ...
  private calculateDeficit(ingredient: RequiredIngredient, pantry: PantryItem | undefined): number {
    ...
    if (pantry.quantity != null && ingredient.quantity > 0) {
      const deficit = ingredient.quantity - pantry.quantity;
