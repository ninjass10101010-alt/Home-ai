...
export function useGrocery(showToast: (msg: string) => void, plannedMeals: Meal[] = []) {
  ...
  // without a manual reload. Same pattern as useMeals/useRecipes.
  useEffect(() => {
    const onRefreshed = () => pullGrocery();
  ...
  const updateGroceryItem = async (id: number | string, updates: { name?: string; quantity?: string; notes?: string; store?: string }) => {
    const item = groceryItems.find(i => i.id === id);
    if (!item) return;
    // Partial updates allowed: the store picker and price-compare sheet send
    // { store } only. Fall back to the current row for omitted fields.
    const nextName = updates.name !== undefined ? updates.name.trim() : item.name;
    if (!nextName) return;
    const next = {
      ...item,
      name: nextName,
      quantity: updates.quantity !== undefined ? updates.quantity.trim() : item.quantity,
      notes: updates.notes !== undefined ? updates.notes.trim() : item.notes,
      ...(updates.store !== undefined ? { store: updates.store } : {}),
    };
    setGroceryItems(prev => prev.map(i => i.id === id ? next : i));
    try { await db.upsertGroceryItem({ ...next, id }); } catch { /* best-effort */ }
  };
