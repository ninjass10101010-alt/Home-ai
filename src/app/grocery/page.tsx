...
export default function GroceryPage() {
  ...
  const [grocerySyncStatus, setGrocerySyncStatus] = useState("");
  const [isGrocerySyncing, setIsGrocerySyncing] = useState(false);
  const [groceryNotification, setGroceryNotification] = useState<string | null>(null);

  // Helper to map db grocery (which may lack emoji) to UI shape with correct emoji + sync fields
  const mapDbToGroceryItem = (g: any): GroceryItem => {
    const cat = g.category || "pantry";
    const categoryDef = groceryCategories.find((c) => c.id === cat);
    return {
      id: g.id,
      name: g.name,
  ...
  useEffect(() => {
    const loaded = db.selectGrocery();
    if (loaded && loaded.length > 0) {
      setItems(loaded.map(mapDbToGroceryItem));
    } else {
      // fallback only if db empty
      setItems(initialGroceryItems);
    }
  }, []);
  ...
  // Sync from pantry handler (import service, persist via it, show status/loading, refresh)
  const handleSyncFromPantry = async () => {
    if (isGrocerySyncing) return;
    setIsGrocerySyncing(true);
    setGrocerySyncStatus("Syncing...");
    setGroceryNotification(null);
    try {
      const result = await mealSyncService.syncPantryToGrocery("demo");
      const msg = `Synced: ${result.updated} updated`;
      setGrocerySyncStatus(msg);
      const fresh = db.selectGrocery().map(mapDbToGroceryItem);
      setItems(fresh.length ? fresh : items);
      setGroceryNotification(`✅ Pantry sync: ${result.updated} items affected`);
      setTimeout(() => setGrocerySyncStatus(""), 2500);
      setTimeout(() => setGroceryNotification(null), 3500);
    } catch (e: any) {
      const err = "Error: " + (e?.message || "sync failed");
      setGrocerySyncStatus(err);
      setGroceryNotification(`❌ ${err}`);
      setTimeout(() => setGrocerySyncStatus(""), 3000);
    } finally {
  ...
  const toggleNeeded = (id: number) => {
    ...
    db.upsertGroceryItem({ ...item, needed: newNeeded, id: item.id });
    const fresh = db.selectGrocery().map(mapDbToGroceryItem);
    setItems(fresh.length ? fresh : items);
  };
  ...
  const toggleOverride = (id: number) => {
    const item = items.find(i => i.id === id);
    const newOverride = !item?.manualOverride;
    db.toggleGroceryOverride(id, newOverride);
    const fresh = db.selectGrocery().map(mapDbToGroceryItem);
    setItems(fresh.length ? fresh : items);
  };
  ...
  const setPriority = (id: number, priority: "low" | "medium" | "high") => {
    const item = items.find((i) => i.id === id);
    if (!item) return;
    db.upsertGroceryItem({ ...item, priority, id: item.id });
    const fresh = db.selectGrocery().map(mapDbToGroceryItem);
    setItems(fresh.length ? fresh : items);
  };
  ...
  const addItem = () => {
    ...
    if (!trimmed) return; // basic validation no empty
    const categoryDef = groceryCategories.find((c) => c.id === newItemCategory);
  ...
  const updateItem = (id: number) => {
    ...
    const item = items.find((i) => i.id === id) || { id };
    db.upsertGroceryItem({
      ...item,
      id,
      name: trimmedName,
  ...
  const deleteItem = (id: number) => {
    ...
    const fresh = db.selectGrocery().map(mapDbToGroceryItem);
    setItems(fresh.length ? fresh : items.filter((i) => i.id !== id));
  };
  ...
  return (
    <PageShell>
      ...
      {/* List Actions */}
      <div className="px-4 mt-4">
        {/* Sync from Pantry button + recently affected by sync note (using lastSyncedAt) */}
        <div className="mb-2">
          <button
            onClick={handleSyncFromPantry}
            disabled={isGrocerySyncing}
            className="w-full py-2 rounded-lg bg-nori-500/15 text-nori-400 text-sm font-medium hover:bg-nori-500/25 disabled:opacity-60 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-1.5"
          >
            🔄 {isGrocerySyncing ? "Syncing from Pantry..." : "Sync from Pantry"} {grocerySyncStatus && `(${grocerySyncStatus})`}
          </button>
          {groceryNotification && (
            <p className={`mt-1 text-[10px] ${groceryNotification.includes("❌") ? "text-rose-400" : "text-nori-400"}`}>{groceryNotification}</p>
          )}
          {items.some((i) => i.lastSyncedAt) && (
            <p className="mt-1 text-[10px] text-text-muted text-center">
              {items.filter((i) => i.lastSyncedAt).length} recently affected by pantry sync
            </p>
          )}
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => {
              // Mark all as needed - persist
              items.forEach((item) => db.upsertGroceryItem({ ...item, needed: true, id: item.id }));
              const fresh = db.selectGrocery().map(mapDbToGroceryItem);
              setItems(fresh.length ? fresh : items);
            }}
          ...
          <button
            onClick={() => {
              // Clear completed items - persist by deleting non-needed
              const toRemove = items.filter((item) => !item.needed);
              toRemove.forEach((item) => db.deleteGroceryItem(item.id));
              const fresh = db.selectGrocery().map(mapDbToGroceryItem);
              setItems(fresh.length ? fresh : items.filter((i) => i.needed));
            }}
            className="flex-1 py-2 rounded-lg bg-surface-2 text-text-secondary text-sm font-medium hover:bg-nori-500/15 hover:text-nori-400 transition-colors"
          >
            Clear Completed
