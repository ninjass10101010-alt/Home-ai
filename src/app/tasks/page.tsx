  ...
  const submitPin = () => {
    ...
    if (task.universal) {
      ...
      if (verified) {
        ...
        fetch("/api/leaderboard", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ memberName: claimantName, points: task.points }) });
    ...
    if (verified) {
      ...
      fetch("/api/leaderboard", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ memberName: (verified as any).name, points: task.points }) });
