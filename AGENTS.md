...
- **Last Updated:** 2026-06-15 | **Google Calendar integration shipped (live & verified).** The previous `public/google-events.json` / `public/google-tasks.json` static-JSON stub is gone. Real Google Calendar API v3 now backs the Calendar tab via PocketBase-backed caching (`consuela_google_calendar_events` collection) + 5-minute client-side polling. New `Settings → Integrations` card with **Google Device Flow** (no public domain required — works on the LAN). Tokens are AES-256-GCM encrypted at rest in `consuela_google_tokens` PB collection. Calendar events appear in the dashboard with member=Google + cyan rail + 📅 emoji; tapping the edit pen on a Google event opens it in `calendar.google.com`. **Google Tasks + Reminders are wired but paused** — Google's Device Flow allowed-scopes list does NOT include the Tasks API (`googleapis.com/auth/tasks` returns `Invalid device flow scope` from `/device/code`). Tasks/Reminders code is complete and ready; the moment the OAuth client is upgraded to "Web" type with a public redirect URI (Tailscale Funnel, Cloudflare Tunnel, or ngrok paid tier), adding the Tasks scope to `GOOGLE_OAUTH_SCOPES` will activate the Tasks/Reminders UI. Until then the Reminders section on the Tasks tab shows a clear "paused — Tasks scope not granted" notice pointing at the next step. **6 PB collections created** with `idempotent` field-aware auto-creation (skips if name exists, patches missing fields if shape changed). **20/20 unit tests pass** (12 encryption + 8 device-flow). `tsc --noEmit` clean, `npm run lint` clean, `npm run build` clean. Phase 2 (Spoonacular, Open-Meteo weather, Web Push) and Phase 3 (Telegram bot, OpenRouter fallback) are queued.
