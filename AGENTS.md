...
- **Last Updated:** 2026-07-17 | **PocketBase migration complete.** All 8 phases of the localStorage→PB migration have been implemented. PB is now the primary data source. localStorage is retained as per-device cache/fallback. A global `CacheRefresher` component polls PB every 60s and on visibility change for cross-device sync. **18 PB collections** auto-created with idempotent field-aware seeding (`seedCollections` patches missing fields on existing collections). Key changes per phase: (2) member writes now re-fetch from PB, (3) `CacheRefresher` for events/schedules/emergency, (4) grocery/pantry init flipped to PB-first, (5) meals/recipes wired to PB writes, (6) tasks/weekData/rewards/penalties/goals/HOF sync to structured PB collections every 5s, (7) auth sessions persist to PB for cross-device login restore, (8) emergency contacts refresh from PB. **Bulk push-to-PB** utility (`src/lib/push-local-to-pb.ts`) with Settings button (Data & sync → "☁️ Push local data to cloud") reads 13 localStorage keys and pushes all existing data to PB via upsert/insert methods. Tasks/leaderboard auto-syncs every 5s; all other domains pushed on demand. | **Interaction consistency (Stream C) delivered 2026-07-17:** a single `.tap` / `.tap-sm` utility class now bundles the full tap language across every tappable surface — `active:scale-[0.97]` press, `hover:scale-[1.02]` (1.01 on small controls) lift, and `focus-visible` accent ring. Replaces the old mixed `active:scale-95/90`, `hover:scale-105`, `hover:brightness-110`, and missing rings. Applied to global chrome (BottomNav, ConsuelaFAB, EmergencyButton, Stepper, SegmentedControl), core UI kit (SoftButton, IconButton, Chip, ListRow, Surface, ThemeToggle, TopBar, Toggle), and Kitchen controls (Meals/Grocery/Pantry/Recipes buttons, day pickers, checkboxes, sync buttons). Respects `prefers-reduced-motion` (scale disabled, ring kept). | **Kitchen flow (Stream B) redesign delivered 2026-07-17** (see above). | **Google Calendar integration** remains live & verified. Google Tasks + Reminders still paused (Device Flow allowed-scopes excludes tasks API). **20/20 unit tests pass** (12 encryption + 8 device-flow). `tsc --noEmit` clean, `npm run lint` clean, `npm run build` clean.
